...
export const InspectorSidebar: React.FC<InspectorSidebarProps> = ({
  ...
  return (
    <div className="relative flex h-full select-none shrink-0 font-sans z-30">
        ...
        >
          ...
          {activeTab === 'catalog' && (
            <div className="flex-1 overflow-y-auto flex flex-col custom-scrollbar">
              {/* Search & Add New Item Header */}
              <div className="p-2.5 border-b border-inherit space-y-2">
                {onOpenAddItemModal && (
                  <button
                    onClick={onOpenAddItemModal}
                    className="w-full py-1.5 px-3 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold flex items-center justify-center gap-1.5 transition shadow-xs hover:scale-[1.01] active:scale-[0.99] cursor-pointer"
                    title="Sculpt or import custom 3D furniture item"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    <span>+ Sculpt / Import New 3D Item</span>
                  </button>
                )}
                ...
                {filteredCatalogItems.length === 0 ? (
                  ...
                  filteredCatalogItems.map((item) => (
                    <div
                      ...
                      className={`p-2 rounded-xl border transition-all duration-150 flex items-center justify-between gap-3 group cursor-grab active:cursor-grabbing ${
                        isDark
                          ? 'bg-slate-900 hover:bg-slate-800/90 border-slate-800 hover:border-indigo-500/50 shadow-xs'
                          : 'bg-white hover:bg-slate-50/80 border-slate-200/90 hover:border-indigo-400 hover:shadow-md shadow-2xs'
                      }`}
                      title="Click to view 360° 3D Preview & materials"
                    >
                      {/* Left: Real 3D Image Display on Pure White Background */}
                      <div
                        className="w-14 h-14 bg-white rounded-lg border border-slate-100 p-1 flex items-center justify-center shrink-0 shadow-2xs group-hover:scale-105 transition-transform duration-200"
                        title="Click to view full 360° 3D Preview"
                      >
                        <CatalogThumbnail
                          icon={item.icon}
                          name={item.name}
                      ...
                      <div className="min-w-0 flex-1">
                        <h4 className="text-xs font-bold text-slate-800 dark:text-white truncate leading-tight group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition">
                          {item.name}
                        </h4>
                        <p className="text-[10px] text-slate-400 font-mono mt-0.5">
                          {item.width} × {item.depth} × {item.height} cm
                        </p>
                        <span className="inline-block mt-1 text-[9px] font-semibold text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded uppercase tracking-wider">
                          {item.category}
                        </span>
                      </div>

                      {/* Right: Add Icon Button (+ 3D Preview Eye) */}
                      <div className="flex items-center gap-1.5 shrink-0">
                        ...
                        <button
                          ...
                          className="w-7 h-7 rounded-lg border border-slate-200 dark:border-slate-700 hover:border-indigo-400 text-slate-500 hover:text-indigo-600 dark:text-slate-400 dark:hover:text-indigo-400 bg-slate-50 dark:bg-slate-800 hover:bg-indigo-50/60 flex items-center justify-center transition cursor-pointer"
                        ...
                        {onAddItem && (
                          <button
                            ...
                            className="w-8 h-8 rounded-lg bg-indigo-600 hover:bg-indigo-500 text-white flex items-center justify-center transition shadow-xs hover:scale-105 active:scale-95 cursor-pointer shrink-0"
      ...
      {/* 3D Catalog Model Interactive Preview & Spec Studio Modal */}
